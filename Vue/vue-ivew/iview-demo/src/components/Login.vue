      ...
      <FormItem>
        <Row>
          <i-col :xs='{ span:4, offset: 6}'>
            <Button type="error" @click="formLoginReset('formLogin')">重置</Button>
          </i-col>
          <i-col :xs='{ span:4, offset: 6}'>
...
export default {
  ...
  data () {
    return {
      ...
      formLoginRules: {
        username: [
          { required: true, message: '用户名为空，请输入用户名...', trigger: 'blur' }
        ],
        password: [
          { required: true, message: '密码为空，请输入密码...', trigger: 'blur' },
          { type: 'string', min: 6, message: '密码最少6位数...', trigger: 'blur' }
        ]
      }
    }
  },
