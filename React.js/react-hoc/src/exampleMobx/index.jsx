        ...
        <ul>
          {
            store.todos.map((todo, index) => {
              return <li key={ index } style={ { textDecoration: todo.completed ? 'line-through' : ''}} onClick={ () => store.toggleTodo(todo.id) } >
                { todo.text }
              </li>
            })
          }
        </ul>
        { store.completedNumber }/{ store.totalNumber }
      </React.Fragment>
    )
  }
}
